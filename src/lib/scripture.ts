 ...
 */
export const detectScriptureReferences = (
  text: string,
): ScriptureReference[] => {
...
export const parseReference = (refText: string): ScriptureReference | null => {
  ...
  const verseStart = verseStartStr ? Number.parseInt(verseStartStr) : 1;
  const verseEnd = verseEndStr ? Number.parseInt(verseEndStr) : undefined;
...
export const getDiscussionQuestions = (
  ...
  switch (bookCategory) {
    ...
    case "prophetic":
      questions.push(
        `What was God calling His people to through this passage?`,
      );
      break;
    case "narrative":
