...
const checkUploadRateLimit = (userId: string = "anonymous"): void => {
  ...
  if (userAttempts.count >= MAX_UPLOADS_PER_HOUR) {
    const minutesRemaining = Math.ceil(
      (userAttempts.resetTime - now) / (60 * 1000),
    );
...
export const uploadImage = async (
  ...
  if (!CLOUD_NAME || !UPLOAD_PRESET) {
    ...
    throw new Error(
      "Cloudinary credentials missing. Please check your environment variables.",
    );
  }
  ...
  if (!hasValidExtension) {
    throw new Error(
      "Invalid file extension. Please use .jpg, .png, .gif, or .webp files.",
    );
  }
...
export const deleteImage = async (_publicId: string): Promise<boolean> => {
  console.warn(
    "⚠️ Image deletion requires server-side API. Image will remain in Cloudinary.",
  );
