...
const TestimonyShareModal: React.FC<TestimonyShareModalProps> = ({
  nightMode, isOpen, onClose, testimonyId, testimonyText, profileName,
}) => {
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: nm ? 'rgba(15, 15, 25, 0.95)' : 'rgba(255, 255, 255, 0.95)',
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className={`text-lg font-bold ${nm ? 'text-white' : 'text-black'}`}>Share Testimony</h2>
            </div>
            <button
              onClick={onClose}
              className={`p-1.5 rounded-xl transition-all hover:scale-110 active:scale-95 ${
                nm ? 'hover:bg-white/10 text-white/50' : 'hover:bg-black/5 text-black/50'
              }`}
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="p-5 space-y-4">
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all active:scale-95 ${
              copiedLink
                ...
                : nm ? 'hover:bg-white/[0.06]' : 'hover:bg-black/[0.03]'
            }`}
            style={{
              background: copiedLink
                ? nm ? 'rgba(34,197,94,0.1)' : 'rgba(34,197,94,0.08)'
                : nm ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.5)',
              border: `1px solid ${
                copiedLink
                  ? 'rgba(34,197,94,0.3)'
          ...
          >
            ...
            >
              {copiedLink ? <Check className="w-4.5 h-4.5 text-white" /> : <Link2 className="w-4.5 h-4.5 text-white" />}
            </div>
            <div className="flex-1 text-left">
              <p className={`text-sm font-semibold ${nm ? 'text-white' : 'text-black'}`}>
          ...
          <div>
            <label className={`block text-xs font-semibold mb-2.5 ${nm ? 'text-white/40' : 'text-black/40'}`}>
              Share to
            </label>
            <div className="grid grid-cols-3 gap-2.5">
              ...
              >
                ...
                <span className={`text-xs font-medium ${nm ? 'text-white/60' : 'text-black/60'}`}>X</span>
              </button>
              ...
              >
                ...
                <span className={`text-xs font-medium ${nm ? 'text-white/60' : 'text-black/60'}`}>Facebook</span>
              </button>
              ...
              >
                ...
                <span className={`text-xs font-medium ${nm ? 'text-white/60' : 'text-black/60'}`}>WhatsApp</span>
              </button>
            </div>
          </div>
          ...
          >
            <div
              ...
              style={{
                background: copiedText
                  ? 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)'
                  : nm ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)',
              }}
            >
              {copiedText
                ? <Check className="w-4.5 h-4.5 text-white" />
                : <Copy className={`w-4.5 h-4.5 ${nm ? 'text-white/60' : 'text-black/50'}`} />
              }
            </div>
            <div className="flex-1 text-left">
              <p className={`text-sm font-semibold ${nm ? 'text-white' : 'text-black'}`}>
