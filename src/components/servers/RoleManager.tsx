...
const RoleManager: React.FC<RoleManagerProps> = ({
  ...
  const [editingColorRoleId, setEditingColorRoleId] = useState<string | null>(
    null,
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2
          className={`text-lg font-bold flex-1 ${nm ? "text-white" : "text-black"}`}
        >
          Manage Roles
        </h2>
        <button
      ...
      <div className="flex-1 p-4 space-y-3">
        ...
        {showCreateForm && (
          ...
          >
            ...
            <div>
              <span
                className={`block text-xs mb-2 ${nm ? "text-white/40" : "text-black/40"}`}
              >
                Color
              </span>
                ...
                {ROLE_COLORS.map((color) => (
                  <button
                    ...
                    style={{
                      background: color,
                      boxShadow:
                        newRoleColor === color ? `0 0 16px ${color}60` : "none",
                      border:
                        newRoleColor === color
                          ? `2px solid white`
            ...
            >
              ...
              {renamingRoleId === role.id ? (
                ...
                <div className="flex items-center gap-0">
                  ...
                  >
                    ...
                    />
                    <span
                      className={`flex-1 text-left font-semibold text-sm ${nm ? "text-white" : "text-black"}`}
                    >
                      {role.name}
                    </span>
                    {role.is_default && (
                      <span
                        className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${nm ? "bg-white/10 text-white/40" : "bg-black/5 text-black/40"}`}
                      >
                        default
                      </span>
                    )}
                    {isExpanded ? (
                      <ChevronDown
                        className={`w-4 h-4 ${nm ? "text-white/40" : "text-black/40"}`}
                      />
                    ) : (
                      <ChevronRight
                        className={`w-4 h-4 ${nm ? "text-white/40" : "text-black/40"}`}
                  ...
                  {!role.is_default && onUpdateRole && (
                    <div className="flex items-center gap-0.5 pr-3">
                      ...
                      <button
                        onClick={() =>
                          setEditingColorRoleId(
                            editingColorRoleId === role.id ? null : role.id,
                          )
                        }
                        className={`p-1.5 rounded-xl transition-all hover:scale-105 active:scale-95 ${
                          nm
              ...
              {isExpanded && (
                <div
                  ...
                  style={{
                    borderTop: `1px solid ${nm ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"}`,
                  }}
                >
                  <div className="pt-3">
                    <span
                      ...
                      {PERMISSION_KEYS.map((permKey) => (
                        ...
                        >
                          <input
                            ...
                            style={{ accentColor: "#4F96FF" }}
                          />
                          <span
                            className={`text-sm ${nm ? "text-white/70" : "text-black/70"}`}
                          >
          ...
          <div className="text-center py-12">
            ...
            >
              <Shield
                className="w-8 h-8"
                style={{
                  color: nm ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)",
                }}
              />
            </div>
            <p className={`text-sm ${nm ? "text-white/40" : "text-black/40"}`}>
