...
const ServersTab: React.FC<ServersTabProps> = ({
  ...
  if (!sv.loading && sv.servers.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-20 px-6">
        ...
        <h2
          className={`text-xl font-bold mb-2 ${nightMode ? "text-white" : "text-black"}`}
        >
          No Servers Yet
        </h2>
        <p
          className={`text-center mb-6 max-w-xs ${nightMode ? "text-white/50" : "text-black/50"}`}
        >
          Create a server for your church or community, or join one with an
          invite link.
        </p>
          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                <p
                  className={`text-xs text-center ${nightMode ? "text-white/30" : "text-black/30"}`}
                >
  ...
  const renderContent = () => {
    ...
    if (sv.activeChannelId) {
      const activeChannel = sv.channels.find(
        (c) => c.id === sv.activeChannelId,
      );
      return (
        <ChannelChat
          nightMode={nightMode}
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p
            className={`text-sm ${nightMode ? "text-white/40" : "text-black/40"}`}
          >
  ...
  if (isMobile) {
    return (
      <div
        className={`flex flex-col ${hideServerRail ? "flex-1" : ""} h-full`}
        style={hideServerRail ? {} : { height: "calc(100vh - 120px)" }}
      >
              ...
              >
                ...
                <button
                  ...
                  className={`w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 transition-all active:scale-95 text-xl ${
                    nightMode
                      ? "bg-white/10 text-green-400 hover:bg-green-500/20 hover:rounded-2xl"
                      : "bg-black/5 text-green-600 hover:bg-green-500/10 hover:rounded-2xl"
                  }`}
                >
                  +
                </button>
            ...
            {sv.activeServer && (
              <div className="flex-1 overflow-y-auto">
                <ChannelSidebar
                  {...channelSidebarProps}
                  onOpenSettings={() => {
                    sv.setViewMode("settings");
                    setMobileView("chat");
                  }}
                  onOpenRoles={() => {
                    sv.setViewMode("roles");
                    setMobileView("chat");
                  }}
                  onOpenMembers={() => {
                    sv.setViewMode("members");
                    setMobileView("chat");
                  }}
                  onOpenAuditLog={() => {
                    sv.setViewMode("audit");
                    setMobileView("chat");
                  }}
  ...
  return (
    <div
      className={`flex ${hideServerRail ? "flex-1" : ""} h-full`}
      style={hideServerRail ? {} : { height: "calc(100vh - 120px)" }}
    >
