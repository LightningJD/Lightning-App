    ...
    const loadData = async () => {
      ...
      try {
        ...
        const filteredUsers = usersToShow.filter(
          (u) => u.id !== profile.supabaseId && !friendIds.has(u.id),
        );
        ...
        const enrichedUsers = await Promise.all(
          unblockedUsers.map(async (user) => {
            ...
            if (profile?.supabaseId) {
              mutualFriends = await getMutualFriends(
                profile.supabaseId,
                user.id,
              );
    ...
    const loadPeople = async () => {
      ...
      try {
        ...
        const [churchMembers, fof, nearby] = await Promise.all([
          ...
          friendIds.length > 0
            ? getFriendsOfFriends(profile.supabaseId, friendIds)
            : Promise.resolve([]),
        ...
        for (const u of (nearby || []) as any[]) {
          ...
          if (!blocked && !blockedBy) {
            filteredNearby.push({
              ...
              reason: u.distance_miles
                ? `📍 ${Number.parseFloat(u.distance_miles).toFixed(1)} mi away`
                : "📍 Nearby",
            });
          }
        }
      ...
      <div className="px-4">
        ...
        >
          <div className="flex items-center gap-2 px-4 py-3">
            <Search
              className={`w-5 h-5 ${nightMode ? "text-slate-400" : "text-slate-500"}`}
            />
            <input
              type="text"
              value={searchQuery}
      ...
      {activeDiscoverTab === "home" && (
        <div className="px-4 mb-3">
          {(profile as any)?.church ? (
            <div
              className={`flex items-center gap-2 px-3 py-2 rounded-xl ${nightMode ? "bg-white/[0.04]" : "bg-blue-50/50"}`}
            >
              <span>⛪</span>
              <span
                className={`text-sm font-medium ${nightMode ? "text-slate-200" : "text-slate-800"}`}
              >
                {(profile as any).church.name}
              </span>
              <span
                className={`text-[10px] ${nightMode ? "text-slate-500" : "text-slate-400"}`}
              >
                Church Feed
              </span>
            </div>
            ...
            <div
              className={`p-3 rounded-xl text-center text-sm ${nightMode ? "bg-white/[0.04] text-slate-400" : "bg-blue-50/50 text-slate-500"}`}
            >
      ...
      <div className="px-4 pb-20" key={activeDiscoverTab}>
        {searchQuery ? (
          ...
          isSearching ? (
            ...
            >
              <p
                className={`text-sm ${nightMode ? "text-slate-400" : "text-slate-600"}`}
              >
          ...
          <div className="space-y-4">
            ...
            {pendingRequests.length > 0 && (
              <div>
                <h3
                  className={`text-sm font-bold mb-2 flex items-center gap-2 ${nightMode ? "text-slate-300" : "text-slate-700"}`}
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`font-semibold text-sm truncate ${nightMode ? "text-slate-100" : "text-slate-900"}`}
                          >
                            {sender.display_name || sender.username}
                          </p>
                          <p
                            className={`text-xs truncate ${nightMode ? "text-slate-400" : "text-slate-500"}`}
                          >
                            @{sender.username}
                          </p>
                        </div>
            ...
            {sortedFriends.length === 0 && pendingRequests.length === 0 ? (
              ...
              >
                ...
                >
                  Browse the People tab to discover and connect with believers
                  near you!
                </p>
                <div
                ...
                >
                  ...
                  >
                    💡 Tip: Visit the <span className="font-bold">People</span>{" "}
                    tab to find believers near you
                  </p>
                </div>
              </div>
            ) : (
          ...
          isLoadingPeople ? (
            ...
            >
              ...
              <p
                className={`text-sm mb-6 ${nightMode ? "text-slate-100/80" : "text-black/70"}`}
              >
                Join a church or add friends to discover people in your
                community!
              </p>
            </div>
          ) : (
              ...
              {churchMembersList.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h3
                      className={`text-sm font-bold ${nightMode ? "text-slate-200" : "text-slate-800"}`}
                    >
                      ⛪ Church Members
                    </h3>
                    {churchMembersList.length > 5 && (
                      <button
                        onClick={() =>
                          setExpandedSection(
                            expandedSection === "church" ? null : "church",
                          )
                        }
                      ...
                      >
                        {expandedSection === "church"
                          ? "Show less"
                          : `See all (${churchMembersList.length})`}
                        {expandedSection === "church" ? (
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {friendsOfFriendsList.length > 5 && (
                      ...
                      >
                        {expandedSection === "fof"
                          ? "Show less"
                          : `See all (${friendsOfFriendsList.length})`}
                        {expandedSection === "fof" ? (
                          <ChevronUp className="w-3 h-3" />
                        ) : (
                          <ChevronDown className="w-3 h-3" />
                  ...
                  <div className="space-y-3">
                    {(expandedSection === "fof"
                      ...
                      : friendsOfFriendsList.slice(0, 5)
                    ).map((user) => (
                      <UserCard
                        key={user.id}
                        user={user}
          ...
          isLoadingTestimonies ? (
              ...
              {[1, 2, 3].map((i) => (
                ...
                >
                  <div className="animate-pulse">
                    <div
                      className={`h-4 ${nightMode ? "bg-white/10" : "bg-slate-200"} rounded w-3/4 mb-2`}
                    ></div>
                    <div
                      className={`h-3 ${nightMode ? "bg-white/10" : "bg-slate-200"} rounded w-1/2 mb-4`}
                    ></div>
                    <div
                      className={`h-3 ${nightMode ? "bg-white/10" : "bg-slate-200"} rounded w-full mb-2`}
                    ></div>
                    <div
                      className={`h-3 ${nightMode ? "bg-white/10" : "bg-slate-200"} rounded w-5/6`}
                    ></div>
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-4">
              ...
              {activeDiscoverTab === "home" && trendingTestimony && (
                ...
                >
                  <div
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold mb-3 ${nightMode ? "bg-amber-500/20 text-amber-300" : "bg-amber-100 text-amber-700"}`}
                  >
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1">
                      ...
                      >
                        {trendingTestimony.users?.display_name ||
                          trendingTestimony.users?.username ||
                          "Anonymous"}
                      </button>
                      <p
                  ...
                  <p
                    className={`text-sm leading-relaxed mb-3 ${nightMode ? "text-slate-200" : "text-slate-700"}`}
                  >
                ...
                .map((testimony: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {testimony.lesson && (
                        <div
                          className={`p-3 rounded-lg mb-3 ${nightMode ? "bg-white/5" : "bg-blue-50/50"}`}
                        >
                          <p
                            className={`text-xs font-semibold mb-1 ${nightMode ? "text-slate-300" : "text-blue-900"}`}
                          >
                            💡 Key Lesson:
                          </p>
                          <p
                            className={`text-xs ${nightMode ? "text-slate-200" : "text-blue-800"}`}
                          >
                            {testimony.lesson}
                          </p>
                        </div>
