...
const CosmeticsEditor: React.FC<CosmeticsEditorProps> = ({
  ...
  const updateCosmetic = <K extends keyof PremiumCosmetics>(
    key: K,
    value: PremiumCosmetics[K],
  ) => {
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <Sparkles
            className={`w-5 h-5 ${nm ? "text-yellow-400" : "text-yellow-500"}`}
          />
          <h3
            className={`text-base font-semibold ${nm ? "text-slate-100" : "text-slate-900"}`}
          >
            Customization
          </h3>
        </div>
        <div
          ...
          style={{
            border: `1px dashed ${nm ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)"}`,
          }}
        >
          <Crown
            className={`w-8 h-8 mx-auto mb-2 ${nm ? "text-yellow-400/40" : "text-yellow-500/40"}`}
          />
          <p
            className={`text-sm font-medium ${nm ? "text-white/60" : "text-black/50"}`}
          >
            Unlock custom branding with Premium
          </p>
          <p
            className={`text-xs mt-1 ${nm ? "text-white/30" : "text-black/30"}`}
          >
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        <span
          className={`block text-sm font-semibold ${nm ? "text-white/70" : "text-black/70"}`}
        >
          Icon Animation
        </span>
        <div className="grid grid-cols-2 gap-2">
          {ICON_ANIMATIONS.map(({ value, label, desc }) => (
            ...
            >
              <p
                className={`text-xs font-semibold ${nm ? "text-white" : "text-black"}`}
              >
                {label}
              </p>
              <p
                className={`text-[10px] mt-0.5 ${nm ? "text-white/40" : "text-black/40"}`}
              >
                {desc}
              </p>
            </button>
        ...
        {(cosmetics.icon_animation === "glow" ||
          cosmetics.icon_animation === "shimmer") && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <input
              ...
              onChange={(e) =>
                updateCosmetic("icon_glow_color", e.target.value)
              }
      ...
      <div className="space-y-2">
        ...
        <div className="mt-3">
          <p
            className={`text-xs mb-2 ${nm ? "text-white/30" : "text-black/30"}`}
          >
            Preview
          </p>
          <BrandedTestimonyCard
            nightMode={nm}
            template={
              (cosmetics.testimony_card_template as TestimonyCardTemplate) ||
              "default"
            }
            ...
            testimony={{
              ...
              excerpt:
                "I never thought I would find peace, but through prayer and community...",
              author: "Sarah M.",
              avatar: undefined,
            }}
          />
