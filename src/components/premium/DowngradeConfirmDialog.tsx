...
import { AlertTriangle, X, TrendingDown, Shield, Sparkles, BarChart3 } from 'lucide-react';
      ...
      >
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {stats && (
            ...
            >
              <p className={`text-xs font-medium ${nm ? 'text-blue-300' : 'text-blue-700'}`}>
                Your community has achieved
              </p>
              <div className="flex justify-center gap-4 mt-2">
                {stats.testimonies !== undefined && (
                  <div>
                    <p className={`text-lg font-bold ${nm ? 'text-white' : 'text-black'}`}>{stats.testimonies}</p>
                    <p className={`text-[10px] ${nm ? 'text-white/30' : 'text-black/30'}`}>testimonies</p>
                  </div>
                )}
                {stats.members !== undefined && (
                  <div>
                    <p className={`text-lg font-bold ${nm ? 'text-white' : 'text-black'}`}>{stats.members}</p>
                    <p className={`text-[10px] ${nm ? 'text-white/30' : 'text-black/30'}`}>members</p>
                  </div>
                )}
                {stats.daysActive !== undefined && (
                  <div>
                    <p className={`text-lg font-bold ${nm ? 'text-white' : 'text-black'}`}>{stats.daysActive}</p>
                    <p className={`text-[10px] ${nm ? 'text-white/30' : 'text-black/30'}`}>days active</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* What they'll lose */}
          <div>
            <p className={`text-xs font-semibold mb-2 ${nm ? 'text-white/60' : 'text-black/60'}`}>
              You'll lose access to:
            </p>
            <div className="space-y-2">
              {FEATURES_LOST.map((f, i) => (
                <div key={i} className="flex items-center gap-2">
                  <f.icon className={`w-3.5 h-3.5 ${nm ? 'text-red-400/60' : 'text-red-500/60'}`} />
                  <span className={`text-xs ${nm ? 'text-white/50' : 'text-black/50'}`}>{f.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Access until */}
          <div className={`p-2.5 rounded-lg text-center ${nm ? 'bg-white/5' : 'bg-black/3'}`}>
            <p className={`text-xs ${nm ? 'text-white/40' : 'text-black/40'}`}>
              Premium stays active through <span className="font-semibold">{accessUntil}</span>
            </p>
          </div>
        </div>
