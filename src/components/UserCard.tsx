    ...
    >
      <div className="flex items-start gap-3">
        ...
        >
          ...
          {user.online && (
            <div
              className={`absolute bottom-0 right-0 w-4 h-4 bg-green-500 rounded-full border-2 ${nightMode ? "border-[#0a0a0a]" : "border-white"}`}
            ></div>
          )}
        </button>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1.5 text-[11px]">
            {user.distance &&
              !Number.isNaN(Number.parseFloat(user.distance)) && (
                <div
            ...
            {showReason &&
              ...
              !user.reason.toLowerCase().includes("similar interests") && (
                <span
                  className={`${nightMode ? "text-slate-100" : "text-black"} text-[10px] font-medium`}
                >
