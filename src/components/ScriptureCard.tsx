...
import { Book, MessageSquare, Share2, ChevronDown, ChevronUp } from 'lucide-react';
import {
  ScriptureReference,
  BibleTranslation,
  TRANSLATIONS,
    ...
    >
      ...
      <div className={`px-3 py-2 flex items-center justify-between border-b ${
        ...
        <div className="flex items-center gap-1.5">
          <Book className={`w-3.5 h-3.5 ${nightMode ? 'text-amber-400' : 'text-amber-600'}`} />
          <span className={`text-xs font-bold ${nightMode ? 'text-amber-300' : 'text-amber-800'}`}>
            {reference.fullReference}
          </span>
        </div>
          ...
          {showTranslations && (
            <div
              className={`absolute right-0 top-full mt-1 rounded-lg border overflow-hidden z-50 min-w-[120px] ${
                nightMode ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-amber-200'
              }`}
              ...
              {TRANSLATIONS.map((t) => (
                <button
                  ...
                  className={`w-full px-3 py-1.5 text-xs text-left flex items-center justify-between ${
                    translation === t.id
                      ? nightMode ? 'bg-amber-500/20 text-amber-300' : 'bg-amber-50 text-amber-700'
                      : nightMode ? 'text-slate-100 hover:bg-white/5' : 'text-black hover:bg-amber-50'
                  }`}
                >
                  <span className="font-semibold">{t.name}</span>
                  <span className={`text-[9px] ${nightMode ? 'text-slate-400' : 'text-black/40'}`}>{t.fullName}</span>
                </button>
              ))}
            </div>
          )}
      ...
      <div className="px-3 py-2.5">
        {loading ? (
          <div className={`text-xs italic ${nightMode ? 'text-slate-400' : 'text-amber-600/60'}`}>
            Loading verse...
          </div>
        ) : (
          <p className={`text-sm leading-relaxed italic ${nightMode ? 'text-slate-200' : 'text-amber-900'}`}>
      ...
      <div className={`px-3 py-2 flex items-center gap-2 border-t ${
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold transition-all ${
            showQuestions
              ? nightMode ? 'bg-amber-500/20 text-amber-300' : 'bg-amber-100 text-amber-700'
              : nightMode ? 'text-slate-300 hover:bg-white/5' : 'text-amber-600 hover:bg-amber-50'
          }`}
        >
        ...
        <div className={`px-3 py-2.5 border-t space-y-1.5 ${
          ...
          {questions.slice(0, 4).map((q, i) => (
            <div key={i} className="flex items-start gap-1.5">
              <span className={`text-[10px] font-bold mt-0.5 ${nightMode ? 'text-amber-500' : 'text-amber-500'}`}>
                {i + 1}.
              </span>
              <p className={`text-xs leading-relaxed ${nightMode ? 'text-slate-300' : 'text-amber-800'}`}>
                {q}
              </p>
            </div>
          ))}
