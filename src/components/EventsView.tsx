...
const EventsView: React.FC<EventsViewProps> = ({
  ...
  const handleRSVP = async (eventId: string, status: RSVPStatus) => {
    ...
    if (result) {
      showSuccess(
        `RSVP updated: ${status === "going" ? "Going" : status === "maybe" ? "Maybe" : "Can't make it"}`,
      );
      await loadEvents();
  ...
  if (selectedEvent) {
    ...
    return (
      <div className="py-4 px-4 space-y-4 pb-24">
        ...
        >
          <h2
            className={`text-xl font-bold mb-3 ${nightMode ? "text-slate-100" : "text-black"}`}
          >
            {selectedEvent.title}
          </h2>
          ...
          {selectedEvent.description && (
            <p
              className={`text-sm mb-4 leading-relaxed ${nightMode ? "text-slate-300" : "text-black/70"}`}
            >
              {selectedEvent.description}
            </p>
          )}
          ...
          <div className="space-y-2.5 mb-4">
            <div className="flex items-center gap-2.5">
              <Calendar
                className={`w-4 h-4 flex-shrink-0 ${nightMode ? "text-blue-400" : "text-blue-600"}`}
              />
              <span
                className={`text-sm ${nightMode ? "text-slate-100" : "text-black"}`}
              >
                ...
                >
                  {
                    RECURRENCE_OPTIONS.find(
                      (r) => r.value === selectedEvent.recurrence,
                    )?.label
                  }
                </span>
              </div>
          ...
          {/* RSVP Counts */}
          <div
            className={`flex gap-3 py-3 border-t border-b mb-4 ${nightMode ? "border-white/10" : "border-white/25"}`}
          >
            ...
            {spotsLeft !== null && (
              <div
                className={`text-sm ml-auto ${spotsLeft <= 3 ? "text-red-500 font-semibold" : nightMode ? "text-slate-300" : "text-black/60"}`}
              >
            ...
            {(["going", "maybe", "not_going"] as RSVPStatus[]).map((status) => {
              ...
              const colors: Record<RSVPStatus, string> = {
                going: isActive
                  ...
                  : nightMode
                    ? "bg-white/5 text-slate-100 border-white/10 hover:bg-green-500/20"
                    : "bg-white/80 text-black border-white/30 hover:bg-green-50 shadow-sm",
                maybe: isActive
                  ? "bg-yellow-500 text-white border-yellow-500"
                  : nightMode
                    ? "bg-white/5 text-slate-100 border-white/10 hover:bg-yellow-500/20"
                    : "bg-white/80 text-black border-white/30 hover:bg-yellow-50 shadow-sm",
                not_going: isActive
                  ? "bg-red-500 text-white border-red-500"
                  : nightMode
                    ? "bg-white/5 text-slate-100 border-white/10 hover:bg-red-500/20"
                    : "bg-white/80 text-black border-white/30 hover:bg-red-50 shadow-sm",
              };
          ...
          {rsvpUsers.length > 0 && (
            <div>
              <h3
                className={`text-sm font-semibold mb-2 ${nightMode ? "text-slate-100" : "text-black"}`}
              >
                Attendees
              </h3>
        ...
        <form onSubmit={handleCreateEvent} className="space-y-4">
          ...
          >
            ...
            <div>
              <label
                htmlFor="title"
                className={`block text-xs font-semibold mb-1.5 ${nightMode ? "text-slate-100" : "text-black"}`}
              >
                Event Title *
              </label>
              <input
            ...
            <div>
              <fieldset className="flex flex-col gap-2">
                <legend
                  className={`text-xs font-semibold mb-1.5 ${nightMode ? "text-slate-100" : "text-black"}`}
                >
                  Repeats
                </legend>
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <h3
            className={`text-sm font-bold ${nightMode ? "text-slate-100" : "text-black"}`}
          >
        ...
        <div className="grid grid-cols-7 gap-1 mb-1">
          {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
            <div
              key={i}
              className={`text-center text-[10px] font-semibold py-1 ${nightMode ? "text-slate-400" : "text-black/50"}`}
            >
              {d}
            </div>
          ))}
          ...
          {days.map((day, i) => {
            ...
            return (
              ...
              >
                ...
                {dayEvents.length > 0 && (
                  <div
                    className={`w-1 h-1 rounded-full mt-0.5 ${isToday ? "bg-white" : "bg-blue-500"}`}
                  />
                )}
              </button>
            );
    ...
    <div className="py-4 px-4 space-y-4 pb-24">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {/* View toggle */}
          <div
            className={`flex rounded-lg border overflow-hidden ${nightMode ? "border-white/10" : "border-white/30"}`}
          >
            <button
      ...
      {loading ? (
        <div
          className={`text-center py-8 ${nightMode ? "text-slate-300" : "text-black/60"}`}
        >
          Loading events...
        </div>
      ) : events.length === 0 ? (
        ...
        >
          ...
          <p
            className={`font-bold text-lg mb-1 ${nightMode ? "text-slate-100" : "text-black"}`}
          >
            No upcoming events
          </p>
          <p
            className={`text-sm ${nightMode ? "text-slate-300" : "text-black/60"}`}
          >
            {canCreate
        ...
        <div className="space-y-2.5">
          {events.map((event) => {
            ...
            const spotsLeft = event.max_capacity
              ? event.max_capacity - (event.going_count || 0)
              : null;
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  {/* Date badge */}
                  <div
                    className={`flex flex-col items-center justify-center min-w-[44px] rounded-xl px-2 py-1.5 ${
                      nightMode ? "bg-blue-500/20" : "bg-blue-50"
                    }`}
                  >
                    <span
                      className={`text-[10px] font-bold uppercase ${nightMode ? "text-blue-400" : "text-blue-600"}`}
                    >
                    ...
                    <span
                      className={`text-lg font-bold leading-tight ${nightMode ? "text-blue-300" : "text-blue-700"}`}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`font-semibold text-sm mb-0.5 ${nightMode ? "text-slate-100" : "text-black"}`}
                    >
                      {event.title}
                    </h3>
                    <p
                      className={`text-xs ${nightMode ? "text-slate-300" : "text-black/60"}`}
                    >
