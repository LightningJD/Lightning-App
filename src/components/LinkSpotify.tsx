...
const LinkSpotify: React.FC<LinkSpotifyProps> = ({
  isOpen,
  onClose,
  nightMode,
  userProfile,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (youtubeUrl) {
      const isValidYouTube =
        /^https?:\/\/(www\.)?(youtube\.com|youtu\.be|m\.youtube\.com)\/.+/.test(
          youtubeUrl,
        );
      if (!isValidYouTube) {
        ...
        >
          ...
          <form onSubmit={handleSave} className="p-6 space-y-4">
            ...
            {fetching && (
              ...
              >
                <Loader2
                  className={`w-4 h-4 animate-spin ${nightMode ? "text-blue-400" : "text-blue-600"}`}
                />
                <p
                  className={`text-sm ${nightMode ? "text-blue-300" : "text-blue-700"}`}
                >
                  Fetching song info...
                </p>
              </div>
            ...
            {videoId && !fetching && (songName || songArtist) && (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <Check
                    className={`w-4 h-4 ${nightMode ? "text-green-400" : "text-green-600"}`}
                  />
                  <span
                    className={`text-xs font-medium ${nightMode ? "text-green-400" : "text-green-600"}`}
                  >
                ...
                <div>
                  <label
                    htmlFor="songName"
                    className={`block text-xs font-medium mb-1 ${nightMode ? "text-slate-400" : "text-slate-500"}`}
                  >
                    Song Name
                  </label>
                  <input
