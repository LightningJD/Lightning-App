...
import {
  ...
  getGuestTestimony,
  clearGuestTestimony,
} from "../lib/guestTestimony";
import {
  unlockSecret,
  startTimeBasedSecrets,
  stopTimeBasedSecrets,
  checkTestimonySecrets,
  checkHolidaySecrets,
  checkProfileSecrets,
  checkMilestoneSecret,
...
export interface AppContextType {
  ...
  handleTestimonyQuestionnaireComplete: (data: {
    content: string;
  ...
  reportData: {
    type: "user" | "testimony" | "message" | "group" | null;
    content: { id: string; ownerId?: string; name?: string } | null;
  };
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [nightMode, setNightMode] = useState(
    localStorage.getItem("lightningNightMode") === "true",
  );
  ...
  const [testimonyStartTime, setTestimonyStartTime] = useState<number | null>(
    null,
  );
  ...
  React.useEffect(() => {
    if (userProfile) {
      setLocalProfile((prev: any) => {
        ...
        if (
          prev.hasTestimony !== userProfile.hasTestimony ||
          prev.story?.id !== userProfile.story?.id
        ) {
  ...
  React.useEffect(() => {
    if (
      userProfile?.username &&
      localProfile?.username !== userProfile.username
    ) {
      setLocalProfile((prev: any) =>
        prev ? { ...prev, username: userProfile.username } : null,
      );
    }
  ...
  React.useEffect(() => {
    ...
    document.documentElement.style.backgroundColor = nightMode
      ? darkBg
      : lightBg;
        ...
        if (guestTestimony) {
          ...
          try {
            ...
            if (isMounted)
              updateToError(
                toastId,
                "Testimony saved locally. You can publish it from your profile.",
              );
          }
        }
      }
  ...
  const handlePrivacyToggle = async (
    ...
    const newSettings = { ...previousSettings, [setting]: value };
  ...
  const handleProfileComplete = async (
    ...
    try {
      ...
      if (updated) {
        ...
        if (
          (profileData as any)._coords?.lat &&
          (profileData as any)._coords?.lng
        ) {
          try {
        ...
        updateToSuccess(
          toastId,
          "Profile setup complete! Welcome to Lightning!",
        );
        setProfileCompleted(true);
        setShowProfileWizard(false);
  ...
  const handleProfileEdit = async (profileData: any): Promise<void> => {
    ...
    try {
      ...
      if (profileData.churchName !== undefined)
        dbData.church_name = profileData.churchName;
      if (profileData.churchLocation !== undefined)
        dbData.church_location = profileData.churchLocation;
      if (profileData.denomination !== undefined)
        dbData.denomination = profileData.denomination;
      if (profileData.yearSaved !== undefined)
        dbData.year_saved = profileData.yearSaved;
      if (profileData.isBaptized !== undefined)
        dbData.is_baptized = profileData.isBaptized;
      if (profileData.yearBaptized !== undefined)
        dbData.year_baptized = profileData.yearBaptized;
      if (profileData.favoriteVerse !== undefined)
        dbData.favorite_verse = profileData.favoriteVerse;
      if (profileData.favoriteVerseRef !== undefined)
        dbData.favorite_verse_ref = profileData.favoriteVerseRef;
      if (profileData.faithInterests !== undefined)
        dbData.faith_interests = profileData.faithInterests;
      ...
      if (updated) {
        ...
        if (profileData._coords?.lat && profileData._coords?.lng) {
          try {
            await updateUserLocation(
              userProfile.supabaseId,
              profileData._coords.lat,
              profileData._coords.lng,
            );
            savedCoords = {
              lat: profileData._coords.lat,
              lng: profileData._coords.lng,
            };
          } catch (locErr) {
        ...
        setLocalProfile((prev: any) => {
          ...
          if (profileData.username !== undefined)
            next.username = profileData.username;
          if (profileData.bio !== undefined) next.bio = profileData.bio;
          if (profileData.location !== undefined)
            next.location = profileData.location;
          if (profileData.avatar !== undefined)
            next.avatar = profileData.avatar;
          if (profileData.avatarUrl !== undefined)
            next.avatarImage = profileData.avatarUrl;
          if (savedCoords) {
          ...
          if (profileData.churchName !== undefined)
            next.churchName = profileData.churchName;
          if (profileData.churchLocation !== undefined)
            next.churchLocation = profileData.churchLocation;
          if (profileData.denomination !== undefined)
            next.denomination = profileData.denomination;
          if (profileData.yearSaved !== undefined)
            next.yearSaved = profileData.yearSaved;
          if (profileData.isBaptized !== undefined)
            next.isBaptized = profileData.isBaptized;
          if (profileData.yearBaptized !== undefined)
            next.yearBaptized = profileData.yearBaptized;
          if (profileData.favoriteVerse !== undefined)
            next.favoriteVerse = profileData.favoriteVerse;
          if (profileData.favoriteVerseRef !== undefined)
            next.favoriteVerseRef = profileData.favoriteVerseRef;
          if (profileData.faithInterests !== undefined)
            next.faithInterests = profileData.faithInterests;
          return next;
        });
        if (
          profileData.testimonyContent &&
          userProfile.story?.id &&
          userProfile.supabaseId
        ) {
        ...
        if (
          ...
          profileData.avatarEmoji !== userProfile.avatar
        ) {
  ...
  const handleEditTestimony = async () => {
    ...
    try {
      ...
      showError(
        error instanceof Error
          ? error.message
          : "Failed to load testimony. Please try again.",
      );
    }
  };
  ...
  const handleTestimonyQuestionnaireComplete = async (testimonyContent: {
    content: string;
    answers: TestimonyAnswers;
    visibility?: "my_church" | "all_churches" | "shareable";
  }): Promise<void> => {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (!targetUserId) {
          updateToError(
            toastId,
            "Failed to identify user profile. Please refresh and try again.",
          );
          return;
        }
        ...
        if (profanityResult.severity === "high") {
          updateToError(
            toastId,
            "Your testimony contains content that violates community guidelines. Please revise and try again.",
          );
          return;
        }
      ...
      updateToError(
        ...
        error instanceof Error
          ? error.message
          : "Failed to save testimony. Please try again.",
      );
    }
  };
  ...
  const value: AppContextType = {
    ...
    notificationCounts,
    handleConversationsCountChange,
    privacySettings,
    notificationSettings,
    searchRadius,
    setSearchRadius,
    ...
    showTestimonyEdit,
    setShowTestimonyEdit,
    testimonyData,
    showTestimonyQuestionnaire,
    setShowTestimonyQuestionnaire,
    testimonyStartTime,
    setTestimonyStartTime,
    handleTestimonyQuestionnaireComplete,
    handleEditTestimony,
    handleTestimonySave,
    handleProfileComplete,
    handleSkipProfileWizard,
    handleProfileEdit,
    showMenu,
    setShowMenu,
    showLogoutConfirm,
    ...
    setShowSaveTestimonyModal,
    showSecretsMuseum,
    setShowSecretsMuseum,
    showBugReport,
    setShowBugReport,
    showTerms,
    setShowTerms,
