  ...
  const detect = useCallback(async (): Promise<GeolocationResult> => {
    ...
    try {
      ...
      try {
        ...
        setError(
          "Could not look up your city. Coordinates saved — you can type the city name manually.",
        );
      }
      ...
      if (err instanceof GeolocationPositionError) {
        switch (err.code) {
          case err.PERMISSION_DENIED:
            setError(
              "Location permission denied. Please enable location access or type your city manually.",
            );
            break;
          case err.POSITION_UNAVAILABLE:
            setError(
              "Could not determine your location. Please type your city manually.",
            );
            break;
          case err.TIMEOUT:
            setError(
              "Location request timed out. Please try again or type your city manually.",
            );
            break;
        }
